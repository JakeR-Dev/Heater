...
export const MostReliable = ({results}) => {
  const mostReliablePlayers = getMostReliablePlayers(results);
              ...
              return (
                <div className="player p-4 border border-white rounded" key={player?.playerId}>
                  ...
                  <div className="player-stats grid grid-cols-2 md:grid-cols-3 gap-2 text-left">
                    ...
                    <p className="col-span-full">Total Minutes Played: <i className="block text-sm text-silver">(avg time per game * games played)</i> <b className="block">{player?.totalMinutesPlayedDisplay || Number(player?.totalMinutesPlayed || 0).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</b></p>
                  </div>
                </div>
              );
            })}
